...
export function CompanyIncorporationForm({
  ...
  const [hasAdditionalIndividuals, setHasAdditionalIndividuals] =
    useState(false);
        ...
        if (response.ok) {
          ...
          if (draftData) {
            setFormData((prev) => ({
              ...
              sourceOfFunds: orObject(draftData.sourceOfFunds, {
                totalAmount: "",
                compositionDetails: "",
              }),
              ...
              purposeOfCompany:
                draftData.purposeOfCompany ?? prev.purposeOfCompany,
              geographicProfile:
                draftData.geographicProfile ?? prev.geographicProfile,
              authorizedShares:
                draftData.authorizedShares ?? prev.authorizedShares,
              ...
              complexStructureNotes:
                draftData.complexStructureNotes ?? prev.complexStructureNotes,
            }));
  ...
  useEffect(() => {
    if (isDev) return; // 🔥 Disable autosave in development
  ...
  const nextStep = () => {
    if (validateCurrentStep()) {
      ...
      setError(
        `Please complete all required fields in the ${FORM_STEPS[currentStep].title} section`
      );
    }
  };
  ...
  const validateCurrentStep = () => {
    ...
    switch (currentStepConfig.id) {
      case "company-name":
        return (
          formData.companyNames.firstPreference &&
          formData.companyNames.chosenEnding
        );
      case "purpose":
      ...
      case "shareholders":
        ...
        return (
          ...
          formData.shareholders.every(
            (sh) => sh.fullName && sh.sharesPercentage && sh.address
          ) &&
      ...
      case "share-capital":
        return (
          formData.authorizedShares === "50000" ||
          (formData.authorizedShares === "custom" &&
            formData.customShares &&
            formData.customParValue)
        );
      case "seals":
        return true; // Always valid since it has default values
      case "source-funds": {
        const { totalAmount, compositionDetails } = formData.sourceOfFunds;
        return Boolean(totalAmount?.trim() && compositionDetails?.trim());
      }
      case "records": {
        const loc = formData.recordsLocation.financialRecordsLocation?.trim();
        const maintainer =
          formData.recordsLocation.financialRecordsMaintainedBy?.trim();

        // Forbidden entries
        const forbidden = ["registered office", "registered agent", "ro", "ra"];

        const invalid = forbidden.some((w) => loc?.toLowerCase().includes(w));

        return !!loc && !!maintainer && !invalid;
      }
      case "declaration":
        return (
          formData.declaration.completedByName && formData.declaration.signature
        );
      default:
        return true;
    }
  ...
  async function saveDrawnSignature() {
    ...
    try {
      ...
      // If we somehow don't already have a preview from stopDrawing,
      // also generate a data URL from the file as a fallback.
      if (!signaturePreview) {
  ...
  const handleIndividualChange = (
    index: number,
    field: keyof RelevantIndividual,
    value: any
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSourceOfFundsChange = (
    field: keyof typeof formData.sourceOfFunds,
    value: string
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleRecordsLocationChange = (
    field: keyof typeof formData.recordsLocation,
    value: string
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleDeclarationChange = (
    field: keyof typeof formData.declaration,
    value: string | boolean
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const removeRelevantIndividual = (index: number) => {
    setFormData((prev) => ({
      ...prev,
      relevantIndividuals: prev.relevantIndividuals.filter(
        (_, i) => i !== index
      ),
    }));
  };
  ...
  const handleSubmit = async (source: "button", e?: React.SyntheticEvent) => {
    ...
    if (FORM_STEPS[currentStep]?.required && !validateCurrentStep()) {
      setError(
        `Please complete all required fields in ${FORM_STEPS[currentStep].title} section`
      );
      return;
    }
  ...
  useEffect(() => {
    if (
      ...
      // assume backend gives us a usable URL/path in `signature`
  ...
  const forbiddenWords = ["ro", "ra", "registered office", "registered agent"];

  const invalidFinancialLocation = forbiddenWords.some((w) =>
    formData.recordsLocation.financialRecordsLocation?.toLowerCase().includes(w)
  );
    ...
    <Card className="max-w-6xl mx-auto">
      <CardHeader>
        <CardTitle className="!text-2xl !font-bold">
          Company Incorporation Form - {jurisdictionName}
        </CardTitle>
        <CardDescription>
        ...
        >
          ...
          {currentStep === 0 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Please list three names for the company in order of preference.
                We will use the first name that is approved. Please also insert
                your desired ending which must be chosen from the word(s)
                "Limited"," Corporation"," Incorporated","Societe Anonyme",
                "Sociedad Anonima"; the abbreviations "Ltd", "Corp", "Inc",
                "S.A." or such other word(s) or abbreviations thereof.
              </p>
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                BVI Legislation and International Standards dictates that we
                must have a full understanding of the company's activities.
                Please explain in detail the purpose, intended activities and
                typical assets. See examples below for guidance.
              </p>
              <ul className="text-sm text-muted-foreground list-disc list-inside space-y-2">
                <li>
                  For an Investment Holding Company, please specify the type of
                  investments it will be holding, such as bonds, privately held
                  shares in XYZ Limited (detailing the activity of XYZ Limited),
                  or publicly traded securities (detailing custodian and value).
                </li>
                <li>
                  For a consultancy or professional services company, please
                  detail the typical services and customers.
                </li>
                <li>
                  For a property holding company, please advise whether
                  commercial or residential and provide the address.
                </li>
              </ul>
                  ...
                  <div className="border rounded-lg p-6">
                    <div className="space-y-4">
                      <h4 className="font-medium">
                        Do you require Nominee Shareholder?
                      </h4>
                      <p className="text-sm text-muted-foreground">
                        If yes, we will provide nominee services for 1,500 HKD.
                        If no, please provide shareholder details below.
                      </p>
                          ...
                          <input
                            ...
                            onChange={() =>
                              setFormData((prev) => ({
                                ...
                                requiresNomineeShareholder: false,
                                shareholders:
                                  Array.isArray(prev.shareholders) &&
                      ...
                      {shareholdersArr.map((shareholder, index) => (
                        ...
                        >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label htmlFor={`shareholder-name-${index}`}>
                                Full Name *
                              </Label>
                              <Input
                            ...
                            <div className="space-y-2">
                              <Label htmlFor={`shares-percentage-${index}`}>
                                Shares Percentage *
                              </Label>
                              <Input
                          ...
                          <div className="space-y-2">
                            <Label htmlFor={`shareholder-address-${index}`}>
                              Address *
                            </Label>
                            <Textarea
              ...
              return (
                <div className="space-y-6">
                  ...
                  <div className="border rounded-lg p-6">
                    <div className="space-y-4">
                      <h4 className="font-medium">
                        Do you require Nominee Director?
                      </h4>
                      <p className="text-sm text-muted-foreground">
                        If yes, we will provide nominee director services for
                        1,000 HKD. If no, please provide director details below.
                      </p>
                    ...
                    <>
                      ...
                      {shareholdersArr.length > 0 && (
                        <div className="border rounded-lg p-6">
                          <h4 className="font-medium mb-4">
                            Select shareholders who will also be directors:
                          </h4>
                          <div className="space-y-3">
                          ...
                          <p className="text-sm text-muted-foreground mt-3">
                            ✓ Selected shareholders will be automatically added
                            as directors
                          </p>
                        </div>
                      )}
                      ...
                      <div className="border rounded-lg p-6">
                        ...
                        <p className="text-sm text-muted-foreground mb-4">
                          Add directors who are not shareholders from the list
                          above.
                        </p>
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    setFormData((prev) => ({
                                      ...prev,
                                      directors: (Array.isArray(prev.directors)
                                        ? prev.directors
                                        : toArray(prev.directors)
                      ...
                      {directorsArr.length > 0 && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                              ...
                              <li key={director?.id ?? index}>
                                ...
                                {director.isShareholder ? (
                                  <span className="text-green-600">
                                    ...
                                    (Shareholder -{" "}
                                    {
                                      shareholdersArr.find(
                                        (sh) =>
                                          sh.id ===
                                          director.selectedShareholderId
                                      )?.sharesPercentage
                                    }
                                    % shares)
                                  </span>
                          ...
                          <p className="text-xs text-blue-600 mt-2">
                            Total directors: {directorsArr.length}
                          </p>
                        </div>
                      )}
                    </>
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold">5. Geographic Profile</h3>

              <div className="space-y-2">
                <Label htmlFor="geographicProfile">Geographic Profile *</Label>
                <p className="text-sm text-muted-foreground">
                  Please detail all jurisdictions where the company intends to
                  trade, hold assets or invest.
                </p>
          ...
          {currentStep === 5 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">6. Share Capital</h3>
              <p className="text-sm text-muted-foreground">
                If authorized shares are 50,000 or less, the annual licence fee
                is $550.00. If authorized shares exceed 50,000 the annual
                licence fee is $1,350.00. Please indicate desired capital
                structure.
              </p>
              ...
              <div className="space-y-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({
                              ...prev,
                              authorizedShares: "custom",
                            }))
                          }
                          className="h-4 w-4 text-blue-600 focus:ring-blue-500"
                        />
                        <Label
                          htmlFor="standardSharesNo"
                          className="text-sm font-medium leading-none"
                        >
                          No
                        </Label>
                ...
                {formData.authorizedShares === "custom" && (
                  <div className="space-y-4 border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="complexStructureNotes">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-800">
                    <strong>
                      Note: Shares will not be issued to persons under the age
                      of 18. Shares can be issued to a parent, guardian, or
                      legal representative in trust for someone under the age of
                      18.
                    </strong>
                  </p>
                </div>
          ...
          {/* Step 7: Additional Relevant Individuals */}
          {currentStep === 6 && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold">
                8. Additional Relevant Individuals
              </h3>
              <p className="text-sm text-muted-foreground">
                Do you have any additional Ultimate Beneficial Owners (UBO),
                Shareholders, Directors or Other Connected Persons besides
                yourself?
              </p>
              ...
              <div className="border rounded-lg p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Your information is already included from the previous
                      registration. Select "Yes" only if you have additional
                      UBOs, shareholders, or directors.
                    </p>
                  </div>
                  <div className="flex space-x-4">
                    ...
                    <Button
                      type="button"
                      variant={
                        !hasAdditionalIndividuals ? "default" : "outline"
                      }
                ...
                <>
                  {formData.relevantIndividuals.map((individual, index) => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <h4 className="font-medium">
                          Additional Relevant Individual #{index + 1}
                        </h4>
          ...
          {currentStep === 7 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold">
                9. General Source of Funds
              </h3>
              <p className="text-sm text-muted-foreground">
                BVI legislation mandates that we fully understand the source of
                funds. Please advise the total amount of funds expected to be
                injected into the company by all UBO's and give a general
                synopsis in the freeform box below. Note that each UBO is
                required to complete and submit a "Supplementary Source of Funds
                and Tax Residence Declaration".
              </p>

              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="totalFundsAmount">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleSourceOfFundsChange("totalAmount", e.target.value)
                    }
                ...
                <div className="space-y-2">
                  <Label htmlFor="fundsCompositionDetails">
                    Provide details of the overall composition of the funds to
                    be injected into the company *
                  </Label>
                  <Textarea
                ...
                <div className="bg-blue-50 border border-blue-200 p-4">
                  <p className="text-sm text-blue-800">
                    <strong>
                      Note: Each UBO is required to complete and submit a
                      "Supplementary Source of Funds and Tax Residence
                      Declaration" form separately.
                    </strong>
                  </p>
                </div>
              </div>
          ...
          {/* Step 9: Records Location */}
          {currentStep === 8 && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold">
                10. Location of Financial Records
              </h3>

              {/* Virtual office selection */}
              <div className="space-y-2">
                <Label>
                  Would you like a virtual office for USD X per annum?
                </Label>
                ...
                <div className="flex flex-col space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      id="virtualOfficeYes"
                      checked={formData.recordsLocation.virtualOffice === true}
                      onChange={() =>
                        setFormData((prev) => ({
                          ...prev,
                          recordsLocation: {
                            ...prev.recordsLocation,
                            virtualOffice: true, // yes
                          },
                        }))
                      }
                      className="h-4 w-4"
                    />
                    <Label htmlFor="virtualOfficeYes">Yes</Label>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      id="virtualOfficeNo"
                      checked={formData.recordsLocation.virtualOffice === false}
                      onChange={() =>
                        setFormData((prev) => ({
                          ...prev,
                          recordsLocation: {
                            ...prev.recordsLocation,
                            virtualOffice: false, // no
                          },
                        }))
                      }
                      className="h-4 w-4"
                    />
                    <Label htmlFor="virtualOfficeNo">No</Label>
                  </div>
                </div>
              </div>

              {/* Financial Records Location */}
              <div className="space-y-4">
                <h4 className="font-semibold">
                  10.1 Financial Records Location
                </h4>
                <p className="text-sm text-muted-foreground">
                  Please provide the physical address where financial records
                  will be maintained, and the name of the person responsible for
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="financialRecordsLocation">
                      Physical address where financial records will be
                      maintained *
                    </Label>
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        handleRecordsLocationChange(
                          "financialRecordsLocation",
                          e.target.value
                        )
                      }
                      placeholder="Full physical address (RO/RA not permitted)"
                      className={`flex min-h-20 w-full rounded-md border ${
                        invalidFinancialLocation
                          ? "border-red-500"
                          : "border-input"
                    ...
                    {invalidFinancialLocation && (
                      <p className="text-xs text-red-600 font-semibold">
                        ❗ Financial records cannot be kept at the Registered
                        Office (RO) or Registered Agent (RA). Please provide a
                ...
                <div className="bg-blue-50 border border-blue-200 p-3 rounded">
                  <p className="text-sm text-blue-800">
                    <strong>
                      Important: All company financial records must be
                      maintained for a minimum of five (5) years after
                      completion of the relevant transaction or the termination
          ...
          {/* Step 10: Declaration (Information Only) */}
          {currentStep === 9 && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold">11. Declaration</h3>

              <div className="space-y-6 border rounded-lg p-6">
                {/* NOTE ONLY */}
                <p className="text-sm text-muted-foreground">
                  <strong>
                    NOTE: Each UBO MUST submit a Supplementary Source of Funds
                    and Tax Residence Declaration.
                  </strong>
                </p>

                {/* This section intentionally no longer contains:
                    - Authorized persons question
                    - PEP question
                    - Signature block
                    - Completed By Name
                    These all move to Review Step.
                */}

                <div className="bg-gray-50 border border-gray-200 p-4 rounded-md">
                  <p className="text-sm text-gray-700 leading-relaxed">
                    Please proceed to the Review step to confirm all information
          ...
          {currentStep === FORM_STEPS.findIndex((s) => s.id === "review") && (
            <div className="space-y-8">
              <h3 className="text-lg font-semibold">12. Review & Submit</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  />
                  <SummaryRow
                    label="Purpose of company"
                    value={formData.purposeOfCompany}
                  />
                  <SummaryRow
                    label="Geographic profile"
                    value={
                      formData.geographicProfile.length > 0
                        ? formData.geographicProfile.join(", ")
                        : "—"
                    }
                  />
                </div>
                    ...
                    <>
                      ...
                      {shareholdersArr.map((sh, i) => (
                        <div key={sh?.id ?? i} className="text-sm">
                          ...
                          — {sh.sharesPercentage || "0"}% —{" "}
                          {sh.address || "No address"}
                        </div>
                      ))}
                    </>
                  )}
              ...
              <div>
                ...
                {individualsArr.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No additional individuals provided.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {individualsArr.map((ri, i) => (
                      ...
                      >
                        ...
                        <div className="text-muted-foreground">
                          {ri.nationality || "—"} · {ri.passportNumber || "—"} ·{" "}
                          {ri.dateOfBirth || "—"}
                        </div>
                        <div>
                          {ri.isUBO ? "UBO · " : ""}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <SummaryRow
                    ...
                    value={
                      formData.recordsLocation.virtualOffice ? "Yes" : "No"
                    }
                  />
                  <SummaryRow
                    label="Financial records location"
                    value={formData.recordsLocation.financialRecordsLocation}
                  />
                  <SummaryRow
                    label="Financial records maintained by"
                    value={
                      formData.recordsLocation.financialRecordsMaintainedBy
                    }
                  />
              ...
              {/* Declaration (signature required here) */}
              <div className="space-y-6 border rounded-lg p-6 mt-8">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  ...
                  the assets to be introduced into the Company do not derive
                  from any criminal, illegal, money-laundering, terrorist,
                  sanctions busting or other nefarious activities. I/We are
                  aware that anti-money laundering legislation has been passed
                  in the BVI and that I am/We are bound by the statements given
                  herein. I confirm that if requested to do so, I/We will
                  provide CCP with any further evidence as may be required in
                  relation to the verification of the identity or activities of
                  the Relevant Individuals and will promptly inform CCP of any
                  changes in the shareholding, directors, officers or ultimate
                  beneficial owners of the Company. I/We have read, accepted,
                  and hereby agree to execute and be bound by CCP's Standard
                  Terms of Business and pay all invoices rendered pursuant to
                  the standard or agreed fee schedule.
                </p>
